...
from src.evaluator import MarketEvaluator
from src.executor import OrderExecutor
from src.merger import PositionMerger
...
class ArbitrageStrategy:
    ...
    async def run_background_reporting(self, streamer) -> None:
        """Prints live market spreads every 10 seconds."""
        while True:
            await asyncio.sleep(10)
            logger.info("--- Live Market Spread Scan ---")

            for market in self.markets:
                yes_data = streamer.best_prices.get(market["yes"], {})
                no_data = streamer.best_prices.get(market["no"], {})
                yes_ask = yes_data.get("ask", 0)
                no_ask = no_data.get("ask", 0)

                if yes_ask > 0 or no_ask > 0:
                    logger.info(f"Market {market['market_id'][-6:]}: "
                                f"YES @ ${yes_ask:.2f} | NO @ ${no_ask:.2f}")

            for market in self.markets:
                await self._process_market(market, streamer.best_prices)
