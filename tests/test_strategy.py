...
@pytest.fixture
def strategy():
    return ArbitrageStrategy(client=None, markets=[], blockchain=None)

def test_crypto_maker_fee_calculation_at_midpoint(strategy):
    """
    ...
    fee = strategy.calculate_taker_fee(amount, price, rate_type="crypto")
    assert fee == 25.0

def test_fee_rounding_down(strategy):
    """
...
def test_sports_fee_calculation(strategy):
    amount = 100
    price = 0.9  
    # 100 * 0.0175 * (1 - (2 * 0.4)^1)
    # 1.75 * (1 - 0.8) = 1.75 * 0.2 = 0.35
    fee = strategy.calculate_taker_fee(amount, price, rate_type="sports")
